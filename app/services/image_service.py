...
class ImageService:
    ...
    async def edit_images(self, image1_url: str, image2_url: str, custom_prompt: Optional[str] = None) -> Optional[str]:
        try:
           ...
           얼굴 반영 (첫 번째 이미지):
           - 첫 번째 이미지의 눈, 코, 입, 헤어스타일, 머리색만 참고
           - 안경은 실제 착용한 경우에만 유지, 없으면 절대 추가하지 말 것
           - 눈을 크게 묘사하면서 원본 특징 유지
           - 미소녀, 미소년 스타일

           캐릭터 기준 (두 번째 이미지):
           - 의상, 갑옷, 무기, 직업군, 캐릭터 디자인 절대 변경 금지
           - 그림 화풍(굵은 검은 선, 색감, 채색 스타일) 그대로 유지
           - 포즈만 변경하고 의상, 무기, 직업, 화풍은 절대 바꾸지 말 것
